         ...
         <div class="row">
            <div class="col-12">
               ...
               <div class="trash-contact">
                        <a href="{{ url('admin/all-services-trash-list') }}" class="export"><img src="{{ url('public/admin/dist/img/trash.svg') }}"></a>
                    </div>
                  <div class="card-body">
                     <!--start table-->
                     <table class="table table-head-fixed text-nowrap" id="slider_id">
                        ...
                        <tbody>
                           @if(count($all_trash_services) >= 1)
                           @php $count = 1; @endphp
                           @foreach($all_trash_services as $service)
                           <tr>
                              <td>{{ $count }}.</td>
                              <td>{{ $service->title }}</td>
                              ...
                              <td>
                                @if ($service->image)
                                    <img src = "{{ asset('public/uploads/services/' .$service->image) }}" width="80" height="60">
                                @endif
                              </td>
                              ...
                              <td class="project-actions text-left">
                                 ...
                                 <a class="btn btn-danger btn-sm delete_service_parament" data-service_id="{{ $service->id }}"><i class="fas fa-trash" aria-hidden="true"></i>Delete</a>
                              </td>
                           </tr>
                           @php $count++; @endphp
                           @endforeach
